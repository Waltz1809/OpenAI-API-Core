...
into a single lightweight set of functions used by main.py for a simpler structure.
"""
...
from .browser import launch_browser, close_all  # re-exported via consolidated flow
from .auth import login as core_login
from .editor import markdown_to_html, fill_title as _fill_title, set_tinymce_content as _set_tinymce_content
...
async def start_session_and_login_interactive(
    ...
    *,
    ...
    """Launch browser, open login page, fill credentials, wait for user to solve CAPTCHA and login."""
...
def discover_volumes(input_path: str) -> List[Dict[str, Any]]:
    ...
    Structure: input/VolumeFolder/*.md ; if no subfolders but md files, treat root as one volume.
    """
...
def _build_manage_url(novel_id: str, *, base_url: Optional[str] = None) -> str:
...
async def _fill_title_and_submit_createbook(page: Page, volume_name: str) -> None:
    ...
    try:
        ...
        logger.debug("Iframe path failed; fallback main page")
...
async def _submit_form(page: Page, selectors: dict, wait_after_submit_ms: int = 15000) -> None:
    form_frame = page.frame_locator('iframe[name="action"]')
