...
async def set_tinymce_content(
    ...
    await editor_iframe_locator.wait_for(state='attached', timeout=editor_ready_timeout_ms)

    # Try TinyMCE API via parent form frame
    form_frame_obj = page.frame(name="action")
    try:
        if form_frame_obj is not None:
            await form_frame_obj.evaluate(
                "html => { if (window.tinymce && tinymce.activeEditor) { tinymce.activeEditor.setContent(html); tinymce.activeEditor.focus(); } else { throw new Error('TinyMCE not ready'); } }",
                html,
            )
    ...
    # Fallbacks: editor body
    editor_body_locator = form_frame.frame_locator(selectors['editor_iframe']).locator(selectors['editor_body'])
    logger.debug(f"[editor] Waiting for editor body: {selectors['editor_body']}")
    await editor_body_locator.wait_for(state='visible', timeout=editor_ready_timeout_ms)

    try:
        await editor_body_locator.evaluate(
            "(el, html) => { el.focus(); el.innerHTML = html; }",
            html,
        )
    ...
    # Last resorts: paste or text fill
    try:
...
def markdown_to_html(md_text: str, *, img_base_url: Optional[str] = None) -> str:
    ...
    if img_base_url:
        ...
        html = re.sub(r"(<img\s+[^>]*src=[\"'])(.*?)([\"'])", _rewrite_src, html, flags=re.IGNORECASE)
    return html
