"""
Job 4 - Uploader
Consists of:
- Load config.yml
- Discover volumes/chapters
- Open browser, log in
- Create volumes and upload chapters sequentially

Designed to run standalone without CLI args.
"""

from __future__ import annotations

import asyncio
import datetime
import logging
import os
import sys
from pathlib import Path
from typing import Any

import yaml

# ----------------------
# Project root utilities
# ----------------------


def find_project_root() -> Path:
    """Locate the project root (three levels up from this file)."""
    return Path(__file__).resolve().parents[2]


def _script_dir() -> Path:
    return Path(__file__).resolve().parent


# Ensure cwd is project root
PROJECT_ROOT = find_project_root()
os.chdir(PROJECT_ROOT)

# Add local core modules to sys.path
sys.path.append(str(_script_dir() / "core"))

# Local imports
from core.steps import (  # noqa: E402
    start_session_and_login_interactive,
    discover_volumes,
    create_volume_and_get_book_id,
    create_chapters_for_volume_dir,
    close_all,
)

# ----------------------
# Config
# ----------------------


def load_config() -> dict[str, Any]:
    """Load config.yml next to this script; return empty dict if missing/invalid."""
    cfg_path = _script_dir() / "config.yml"
    if not cfg_path.exists():
        print(f"⚠️  Config not found: {cfg_path}")
        return {}
    try:
        with cfg_path.open("r", encoding="utf-8") as f:
            data = yaml.safe_load(f) or {}
        return data if isinstance(data, dict) else {}
    except Exception as e:
        print(f"⚠️  Failed to parse config: {e}")
        return {}


# ----------------------
# Logging
# ----------------------


def setup_logging(
    name: str = "uploader",
    level: str = "INFO",
    log_dir_override: str | None = None,
) -> logging.Logger:
    """Configure console + file logging."""
    logger = logging.getLogger(name)
    if logger.handlers:
        return logger
    logger.setLevel(getattr(logging, level.upper(), logging.INFO))

    # Console handler
    ch = logging.StreamHandler()
    ch.setFormatter(logging.Formatter("%(levelname)s: %(message)s"))
    logger.addHandler(ch)

    # File handler
    try:
        if log_dir_override:
            raw_path = Path(log_dir_override)
            log_dir = raw_path if raw_path.is_absolute() else (PROJECT_ROOT / raw_path)
        else:
            log_dir = PROJECT_ROOT / "inventory" / "logs"
        log_dir.mkdir(parents=True, exist_ok=True)
        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        fh = logging.FileHandler(log_dir / f"uploader_{ts}.log", encoding="utf-8")
        fh.setFormatter(
            logging.Formatter("%(asctime)s | %(levelname)s | %(message)s")
        )
        logger.addHandler(fh)
    except Exception:
        pass

    return logger


# ----------------------
# Path resolving
# ----------------------


def resolve_path_from_repo(path_str: str) -> str:
    """
    Resolve a possibly-relative path from:
      1) src directory (two levels up from this file)
      2) repository root (three levels up)
    Returns the first existing path, otherwise the original string.
    """
    p = Path(path_str)
    if p.is_absolute() and p.exists():
        return str(p)

    bases = [_script_dir().parent, PROJECT_ROOT]
    for base in bases:
        candidate = (base / path_str).resolve()
        if candidate.exists():
            return str(candidate)
    return path_str


# ----------------------
# Main async flow
# ----------------------


async def _amain() -> None:
    cfg = load_config()

    # Setup logger
    paths_cfg = cfg.get("paths") or {}
    log_path_cfg = (paths_cfg.get("log") or "").strip() or None
    logger = setup_logging(log_dir_override=log_path_cfg)
    logger.info("Starting uploader")

    # Config values
    series_cfg = cfg.get("series") or {}
    novel_id = (series_cfg.get("novel_id") or "").strip()
    base_url = (series_cfg.get("base_url") or "").strip() or None
    img_base_url = (series_cfg.get("img_base_url") or "").strip() or None

    if not novel_id:
        logger.error("Missing series.novel_id in config.yml")
        return

    input_path_cfg = (paths_cfg.get("input") or "").strip()
    input_path = resolve_path_from_repo(input_path_cfg) if input_path_cfg else ""
    if not input_path or not os.path.exists(input_path):
        logger.error(f"Invalid or missing paths.input: {input_path_cfg}")
        return
    logger.info(f"Input path: {input_path_cfg} -> {input_path}")

    creds = cfg.get("credentials") or {}
    username = (creds.get("username") or "").strip()
    password = (creds.get("password") or "").strip()
    if not username or not password:
        logger.warning("⚠️ Credentials missing, login may fail")

    selectors = cfg.get("selectors") or None
    if not selectors:
        logger.warning("No custom selectors provided, using defaults")

    # Start browser session
    p, browser, context, page = await start_session_and_login_interactive(
        username=username,
        password=password,
        headless=False,
        channel="msedge",
        base_url=base_url,
    )

    try:
        # Discover volumes
        volumes = discover_volumes(input_path)
        if not volumes:
            logger.warning("No volumes discovered. Nothing to upload.")
            return
        logger.info(f"Discovered {len(volumes)} volume(s)")

        # Sequential per-volume
        for v in volumes:
            vname, vpath = v["name"], v["path"]
            logger.info(f"=== Volume: {vname} ===")
            try:
                # Create volume
                bid = await create_volume_and_get_book_id(
                    page,
                    novel_id=novel_id,
                    volume_name=vname,
                    base_url=base_url,
                )
                if not bid:
                    logger.error(f"Failed to create volume '{vname}', skipping")
                    continue

                # Create chapters
                results = await create_chapters_for_volume_dir(
                    page,
                    novel_id=novel_id,
                    book_id=bid,
                    volume_dir=vpath,
                    base_url=base_url,
                    selectors=selectors,
                    img_base_url=img_base_url,
                )
                total = len(results)
                ok = sum(1 for v in results.values() if v)
                fail = total - ok
                logger.info(f"Volume '{vname}' done: total={total}, ok={ok}, fail={fail}")
            except Exception as e:
                logger.exception(f"Error in volume '{vname}': {e}")
                continue

        await asyncio.sleep(1)
    except Exception as e:
        logger.exception(f"Unhandled error: {e}")
    finally:
        logger.info("Closing browser...")
        await close_all(p, browser, context)


def main() -> None:
    try:
        asyncio.run(_amain())
    except KeyboardInterrupt:
        print("\n⏹️  Stopped.")


if __name__ == "__main__":
    main()
